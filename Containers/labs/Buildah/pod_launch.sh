...
chown -R 27:27 $pathmiovol

# Creating and running the MySQL container
podman run -d --name db -e MYSQL_USER=wordpress -e MYSQL_PASSWORD=wordpress -e MYSQL_DATABASE=wordpress -e MYSQL_ROOT_PASSWORD=rootpwd -v $miovol:/var/lib/mysql/data --pod $miopod registry.access.redhat.com/rhscl/mysql-57-rhel7

# Checking readiness for MySQL service
readiness "MySQL" podman exec db bash -c 'mysql -u root -e "SELECT 1"'

# Creating and running the WordPress container
podman run -d --name wp -e WORDPRESS_DB_HOST=127.0.0.1:3306 -e WORDPRESS_DB_USER=wordpress -e WORDPRESS_DB_PASSWORD=wordpress -e WORDPRESS_DB_NAME=wordpress --pod $miopod quay.io/redhattraining/wordpress:5.3.0

# Checking readiness for WordPress
readiness "WordPress" curl -s -o /dev/null http://127.0.0.1:8880/
